        ...
        for label_name in sorted(os.listdir(img_subdir)):
            ...
            for fname in sorted(os.listdir(img_dir)):
                ...
                if not os.path.isfile(anno_path):
                    logger.warning(
                        f"[MISSING] {anno_path}: annotation file not found, skipping"
                    )
                    continue
...
def get_loaders(data_dir, annotation_dir=None):
    ...
    if annotation_dir:
        ...
        vali_dataset = datasets.ImageFolder(
    ...
    vali_loader = DataLoader(
        vali_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers
    )
