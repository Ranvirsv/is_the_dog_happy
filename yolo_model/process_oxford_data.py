...
YOLO_DATASET_DIR = "./yolo_model/data/yolo_oxford_dataset"
...
def get_annotation_path(image_path):
    ...
    annotation_path = os.path.join(ANNOTATIONS_DIR, image_name)
...
def process_images(image_list, split):
    for image_path in image_list:
        ...
        if annotation_path is None:
            shutil.copy(image_path, os.path.join(YOLO_DATASET_DIR, "images", "test", os.path.basename(image_path)))
        ...
        if not is_valid_yolo_label_file(txt_path, num_classes=2):
...
print("YOLOv8 oxford dataset prepared successfully!")